...
def main():
    ...
    reference_models = ["ts_equal_vol90", "ts_shrink_80_primary_quarterly_vol90", "ts_legacy_optuna",
                        "xs_ic5_20_primary_dollar_neutral", "breakout_32", "breakout_equal",
    ...
    cost_models = ["ts_shrink_80_primary_quarterly_vol90", "xs_ic5_20_primary_dollar_neutral", "breakout_32"]
    ...
    funding_models = [name for name in cost_models + ["breakout_equal"] if name in daily_funding]
    ...
    body = hero("ACAUSAL CAPITAL · CRYPTO MOMENTUM", "Cross-sectional momentum survived execution repair,",
                ...
                ["Data cutoff 18 Sep 2026", "581-contract fitting panel", "97 eligible portfolio assets", "70,780 saved configurations", "Shadow research only"])
    ...
    body += finding(6, "Multiple testing is substantial", "All requested scenarios are retained and ranked by raw net Sharpe, but 70,780 configurations are not independent evidence. Freeze a parsimonious model before prospective logging.")
    ...
    body += '<section id="methods"><h2>What was tested</h2><p><strong>Time-series:</strong> five EWMAC rules; equal, 75/80/90%-shrunk and legacy Optuna weights; 25–30% rule caps; quarterly/semiannual/annual/frozen refits; and 60/90/180/360-day volatility. Pooled calibration uses 581 usable historical contracts including delisted names.</p><p><strong>Cross-sectional:</strong> 1-, 5-, 20-, and equal 5/20-day rank-IC fits; signed, continuous dollar-neutral, 10/20/30% baskets, long-only, BTC hedge, beta constraint, BTC residualization and weekly rank buffers.</p><p><strong>Breakout:</strong> 16/32/64/128/256-day horizons and equal ensemble. The time-series/cross-sectional combination is diagnostic only.</p><p><strong>Portfolio:</strong> 15–40% volatility targets, 1–3× gross, 5–40% ticker caps, daily/weekly/monthly rebalancing, 0/50/100% taker, 0/2/5/10 bps slippage and next-close sensitivity. Headline results use next-open open-to-open P&amp;L, 100% taker and 5 bps.</p>' + figure_html(chart_html(fig_models), "All standalone model variants", "Each point is that model’s validation-best headline risk setting.", "../data_store/crypto_momentum_research/complete_results/tested_configurations.parquet") + '</section>'
